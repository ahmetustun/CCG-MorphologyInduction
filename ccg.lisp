...
(defmacro put-derivative (key der)
  `(setf (rest (gethash ,key *training-hashtable*)) ,der))
(defmacro put-key-counts (key val)
  `(setf (gethash ,key *training-non0-hashtable*) ,val))
...
(defun make-lrule-hashtable ()
  ...
  (make-hash-table :test #'eql :size 100 :rehash-size 2 :rehash-threshold 1.0))
...
(defun make-training-hashtable (n)
  ...
  (make-hash-table :test #'eq :size (+ n 100) :rehash-size 1000 :rehash-threshold 1.0))

;;; =======
;;; globals
;;; =======

(defparameter *hash-data-size* 16796)  ; for CKY and LF argmax tables. Make IT REALLY BIG for training sets
                                       ; involving LOOOONG sentences.
				       ; default is the 11th Catalan number
...
(defvar *cky-hashtable* (make-cky-hashtable *hash-data-size*))    ; this is the CKY table keyed by cky loop indices
(defvar *cky-lf-hashtable* (make-lf-hashtable *hash-data-size*)) ; All LFs for the solution in the cky table.
...
;; for beam search in inside-outside computation (all except *beamp* change dynamically)
(defvar *beamp* nil)          ; to beam or not to beam
(defvar *beam-haltp* nil)   ; whether to cut the beam in the middle (determined dynamically)
(defvar *beam-countp* nil)  ; whether to stop by # analyses
(defvar *beam-weightp* nil) ;   or weights (exclusively set)
(defvar *cky-minprob* most-positive-single-float)      ; current lowest ranking weight in cky table.
(defvar *cky-maxprob* most-negative-single-float)      ; current highest ranking weight in cky table.
(defvar *cky-nparses* 0)    ; current number of parses in result cell in cky table.
(defvar *cky-length* 0)     ; current  number of words in a successful parse.
(defvar *beam-epsilon* 1.0) ; min non-negligible difference in max-min weight ratio
(defvar *beam-weight-threshold* 0.0) ; min weight to make it to beam (dynamic)
(defvar *beam-count-max* 1)          ; max count to be in the beam (dynamic)
...
(defvar *training-non0-hashtable*); parameter vector and current nonzero counts
    ...
    (and (ccg-deduce itemslist) ; this creates the CKY table with its local counts
	 (do ((maxprob most-negative-single-float)
	   ...
	   ((null (gethash (list n 1 k) *cky-hashtable*))
	    ...
	    (setf *cky-length* n)
	    (setf *cky-maxprob* maxprob)
	    (setf *cky-minprob* minprob)
...
(defun set-training-parameters (bign smalln nparams alpha0 c) 
  ...
  (setf *training-non0-hashtable* (make-training-hashtable nparams)) ; for inside-outside
        ;; unlike the first hash table, only non-zero counts will have entry in the second, to cut down looping
...
(defun formula1 (nw na maxw minw)
  (max (expt maxw (* (max (/ (min nw na) (max nw na))) (/ (- (abs maxw) (abs minw)) (abs maxw)))) minw))

(defun formula2 (nw na maxw minw)
  (max  (- maxw (/ (- maxw minw) (expt (/ na nw) 0.33333))) minw))

(defun formula3 (nw na maxw minw)
  (max  (- maxw (/ (- maxw minw) (expt (/ na (sqrt nw)) 0.33333))) minw))

(defun beam-epsilonp (maxw minw na)
  (< (- maxw minw) (* (expt na 0.1) *beam-epsilon*)))  ; wider ranges considered equal if too many analyses

(defun beam (nw na maxw minw)
  "nw : #words na: #analyses maxw: max weight minw: min weight"
  (setf *beam-haltp* nil) ; continue do* of find-derivative unless count is turned on and exceeded
  (if *beamp* 
       (if (beam-epsilonp maxw minw na) ; weights too close: reduce by analysis, and 
                                        ;   reduce more analyses when weight range is negligible
	 (progn 
	   (setf *beam-count-max* (expt na 0.666666666))
	   (setf *beam-countp* t) (setf *beam-weightp* nil))
	 (progn                             ; otherwise reduce by weight threshold
	   (setf *beam-weightp* t) (setf *beam-countp* nil)
	   (setf *beam-weight-threshold* (formula3 nw na maxw minw))))))   ; change the formula if you feel like it
                                                                           ; you need to deliver something btw maxw and minw

(defun inside-outside ()
  "after parses for current training pair are found, this function finds the nonzero counts in them,
  and places only those in the non0 training table."
    ...
    (maphash #'(lambda (key val)
		 (block nonzero
			(do* ((r3 1 (+ r3 1)))
			  ...
			  (if (> (count-feature key (list r1 r2 r3)) 0.0) 
			    (progn 
			      (put-key-counts key t)       ; sets the *training-non0-hashtable* to t
			      (return-from nonzero))))))  ; finding the item in one result is enough; derivative will calculate sums
	     *training-hashtable*)))
...
(defun find-derivative-of-log-likelihood (s-lf verbose debug)
  ...
  (let* ((result (ccg-induce (sup-sentence s-lf)))   ; get all analyses. we will filter later (ie No Normal form parsing)
	 ...
	 (s (and result (inside-outside))) ; produces *training-non0-hashtable*
	 (b (and *beamp* (beam *cky-length* *cky-nparses* *cky-maxprob* *cky-minprob*))) ; args were calcutated by ccg-induce
	                                                                                 ; beam in turn sets beam parameters
	 (r1 (cell-len *cky-max*))
	 (r2 (cell-pos *cky-max*)))
    (cond (result
	    (maphash 
		  ...
		  (do* ((in-sum 0.0)
			...
			(all-li-sum 0.0)
			(r3 1 (+ r3 1)))
		    ((or *beam-haltp* (null (gethash (list r1 r2 r3) *cky-hashtable*))) ; loop for every solution if not cut short
		     (update-derivative key in-sum all-sum li-sum all-li-sum verbose debug))
		    (block analyses
			   (and *beamp* *beam-countp* (> r3 *beam-count-max*)(setf *beam-haltp* t)(return-from analyses)) ; skip and stop
			   (let* ((term-cell (get-cell-param (list r1 r2 r3)))
				  (term-c (count-feature key (list r1 r2 r3)))
			     ...
			     (and *beamp* *beam-weightp* (< term-cell *beam-weight-threshold*)(return-from analyses))       ; skip
			     (cond ((alpha-equivalent lf cell-lf)
				   ...
				   (t 
				     (setf all-sum (+ all-sum term-e))
				     (setf all-li-sum (+ all-li-sum term))))))))
	      *training-non0-hashtable*))  ;; outside counts are zero
...
(defun show-training ()
  ...
  (switches)
  (format t "~%Training parameters: N = ~a alpha0 = ~a c = ~a n = ~a *beamp* = ~A" 
	  *bign*  *alpha0* *c* *smalln* *beamp*)
